...
{
    // Base Case , if flooe==0 or floor==1 , means in worst case scenario , we don't need to check anymore
    if(floor==0 || floor==1)
    {
        return floor;
    }
    ...
    for(int k=1;k<=floor;k++)
    {
        ...
        mn=min(mn,temp);
        // this min for to find out the minimum form the worse case attepmts .
    }
    return mn;
}
