...
type of the ith dish. He wants to choose as many dishes as possible from the given list but 
...
For type 1, Ramu can choose at most four dishes. One of the ways to choose four dishes of 
type 1 is A1,A4, A7 and A9.
...
void solve() {
    ...
    for (auto it1 : s) {
        int idx = -INF, ans = 0;
        for (auto it2 : v[it1]) {
            if (it2 - idx > 1) ans++, idx = it2;
        }
        mx = max(mx, ans);
        m[it1] = ans;
    }
    for (auto it : m) if (it.S == mx) res = min(res, it.F);
    cout << res << '\n';
}
